        ...
        for (let path in oas.paths) {
            for (let method in oas.paths[path]) {
                ...
                if (typeof options.selectQueryOrMutationField === 'object' &&
                    ...
                    typeof options.selectQueryOrMutationField[oas.info.title][path][method] === 'number') {
                    if (options.selectQueryOrMutationField[oas.info.title][path][method] === graphql_1.GraphQLOperationType.Query) {
                        isMutation = false;
                    }
                    else {
                        isMutation = true;
